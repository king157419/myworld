...
function Deck() {
  ...
  // 踏条（装饰）：沿坡面均匀嵌一排薄木条，给"台阶"读感而不破坏连续坡的顺滑。
  ...
  return (
    <group>
      ...
      <mesh geometry={ramp} castShadow receiveShadow>
        <meshStandardMaterial color={PALETTE.woodWarm} roughness={0.72} metalness={0.04} />
      </mesh>

      {/* 踏条：薄木条嵌在坡面上（贴着坡面、随坡倾斜）*/}
      {nosings.map((p, i) => (
        <mesh key={i} position={p} rotation={[SLOPE_ANG, 0, 0]} castShadow>
