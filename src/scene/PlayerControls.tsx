...
export default function PlayerControls() {
  ...
  const beginFocus = useCallback((id: string) => {
    ...
    // 锚点按 zone.type 解析（id 是用户数据，导入的世界可改名）；查无此区时回退持久化的 zone.position。
    ...
    const cx = f ? f.center[0] : a ? a.position[0] : p ? p[0] : 0;
    const cy = f ? f.center[1] : a ? a.position[1] : p ? p[1] : 1.4;
    const cz = f ? f.center[2] : a ? a.position[2] : p ? p[2] : 0;
